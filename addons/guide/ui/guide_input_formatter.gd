...
const CompositeRenderer = preload("renderers/composite/composite_renderer.tscn")
...
var _icon_size: int
var _icon_hints: Dictionary = {} ## Maps action names to icon hints
...
static func _ensure_readiness():
	...
	# Add composite renderer first (highest priority for hints)
	add_icon_renderer(CompositeRenderer.instantiate())
...
## Adds an icon hint for a specific action. The hint will be used when rendering the action's icon.
func add_icon_hint(action: GUIDEAction, hint: GUIDEIconHint) -> void:
	if action != null:
		_icon_hints[action] = hint

## Removes an icon hint for a specific action.
func remove_icon_hint(action: GUIDEAction) -> void:
	_icon_hints.erase(action)

## Gets the icon hint for a specific action, if any.
func get_icon_hint(action: GUIDEAction) -> GUIDEIconHint:
	return _icon_hints.get(action, null)
...
func action_as_richtext_async(action: GUIDEAction) -> String:
	# Check if we have a hint for this action
	var hint = get_icon_hint(action)
	if hint != null:
		return await _render_action_with_hint_async(action, hint)
	return await _materialized_as_richtext_async(_materialize_action_input(action))

func _render_action_with_hint_async(action: GUIDEAction, hint: GUIDEIconHint) -> String:
	_ensure_readiness()

	# Get the action mapping for this action
	var mapping: GUIDEActionMapping = _action_resolver.call(action)
	if mapping == null:
		push_warning("No mapping found for action when rendering with hint")
		return await _materialized_as_richtext_async(_materialize_action_input(action))

	# Find the composite renderer
	var composite_renderer = null
	for renderer in _icon_renderers:
		if renderer is GUIDECompositeRenderer:
			composite_renderer = renderer
			break

	if composite_renderer == null:
		push_warning("No composite renderer available for hint-based rendering")
		return await _materialized_as_richtext_async(_materialize_action_input(action))

	if not composite_renderer.supports_hint(hint, mapping):
		push_warning("Composite renderer doesn't support the provided hint for this action mapping")
		return await _materialized_as_richtext_async(_materialize_action_input(action))

	# Create a temporary input for caching purposes
	var temp_input = GUIDEInputAction.new()
	temp_input.action = action

	var icon: Texture2D = await _icon_maker.make_icon_with_hint(temp_input, composite_renderer, hint, mapping, _icon_size, _icon_renderers)

	if icon == null:
		push_warning("Failed to create composite icon for action: ", action)
		return await _materialized_as_richtext_async(_materialize_action_input(action))
