...
func _on_input_mappings_duplicate_requested(index:int) -> void:
	...
	copy.input = GUIDEEditorUtils.duplicate_if_inline(copy.input)

	for i in copy.modifiers.size():
		copy.modifiers[i] = GUIDEEditorUtils.duplicate_if_inline(copy.modifiers[i])

	for i in copy.triggers.size():
		copy.triggers[i] = GUIDEEditorUtils.duplicate_if_inline(copy.triggers[i])
